...
class List {
    constructor(player) {
        ...
        if(this.player.template.list.style.height === '0px'){
            this.showing = false;
        }
    ...
    add(audios) {
        ...
        if(this.player.options.fixedBar){
            this.player.updateListNum()
        }
        if(this.player.options.storeList){
            this.player.updateListStorage()
        }
    }
    ...
    switch(index) {
        ...
        if (typeof index !== 'undefined' && this.audios[index]) {
            ...
            // set html
            this.player.template.pic.style.backgroundImage = audio.cover ? `url('${audio.cover}')` : `url(${this.player.options.defaultCover})`;
