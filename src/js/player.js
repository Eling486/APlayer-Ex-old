...
class APlayer {
    ...
    constructor(options) {
        ...
        if (this.options.fixedBar && this.options.audio.length > 1) {
            this.updateListNum()
        }

        if(this.options.storeList){
            let listStorage = this.storage.get('list')
            let listIndex = this.storage.get('listIndex')
            if(listStorage){
                this.list.clear()
    ...
    updateListNum(){
        if(this.list.audios.length > 99){
            this.template.listLength.innerHTML = '99+'
        }else{
            this.template.listLength.innerHTML = this.list.audios.length
        }
    }
    
    updateListStorage(){
        this.storage.set('list', this.list.audios)
        this.storage.set('listIndex', this.list.index)
    }
